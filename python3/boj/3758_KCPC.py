...
if __name__ == "__main__":
    ...
    for _ in range(T):
        n, k, t, m = map(int, readline().split())
        teams = [{i: [0, 0, 0] for i in range(1, k + 1)} for _ in range(n + 1)]

        for l in range(m):
            id, pnum, score = map(int, readline().split())

            team = teams[id]
            prevScore, count, _ = team[pnum]
            team[pnum] = [max(prevScore, score), count + 1, l]

        totals = []

        for i, team in enumerate(teams):
            total, totalSubmitCount, lastLog = 0, 0, 0

            for score, count, log in team.values():
                total += score
                totalSubmitCount += count
                lastLog = max(lastLog, log)

            totals.append([total, totalSubmitCount, lastLog, i])

        totals.sort(key=lambda x: (-x[0], x[1], x[2]))
