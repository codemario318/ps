...
def crossCountry(ranks):
    counter = Counter(ranks)
    ranks = filter(lambda x: counter[x] >= LIMIT, ranks)

    scores = dict()
    for rank, team in enumerate(ranks, 1):
    ...
    return min(scores.keys(), key=lambda x: (scores[x][0], scores[x][5]))
